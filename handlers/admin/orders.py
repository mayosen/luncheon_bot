...
def approve_order(update: Update, context: CallbackContext):
    ...
    if order.status != "подтверждение":
        query.message.reply_text(f"Заказ <code>#{order_id}</code> принят другим администратором.")
        return
...
def reject_order(update: Update, context: CallbackContext):
    ...
    if order.status != "подтверждение":
        query.message.reply_text(f"Заказ <code>#{order_id}</code> отклонен другим администратором.")
        return
...
def register(dp: Dispatcher):
    ...
    dp.add_handler(CallbackQueryHandler(pattern=r"^admin:status:", callback=set_order_status))
